...
int main(int argc, char* argv[]){
  ...
  string lumi="goldenJson12p9_new";
  string inputFile="LFV_12p9_ichep";
...
*/
  ...
  cb.cp().process(ch::JoinStr({sig_procs, {"DY", "TT","WG","T","Dibosons","ggHTauTau","vbfHTauTau","WJETSMC","QCD"}}))
...
//      ({"2015"}, 1.12));
  ...
  cb.cp().process({"QCD"})
      .AddSyst(cb, "norm_qcd", "lnN", SystMap<>::init(1.3));
  ...
  cb.cp().process(ch::JoinStr({sig_procs, {"DY", "WG", "TT","T","Dibosons","ggHTauTau","vbfHTauTau","WJETSMC","QCD"}}))
...
//  bbb.MergeBinErrors(cb.cp().backgrounds());
 bbb.MergeBinErrors(cb.cp().process({"Dibosons","DY","WG","T","TT","ggHTauTau","vbfHTauTau","WJETSMC"}));
